...
<style>
:root{--bg:#080c10;--surface:#0d1117;--border:#1e2732;--text:#cdd9e5;--muted:#545d68;--accent:#388bfd;--green:#3fb950;--red:#f85149;--yellow:#d29922;--mono:'IBM Plex Mono',monospace;--sans:'IBM Plex Sans',sans-serif;}
...
<div class="page">
  ...
  <div class="status-row">
    <div class="status-dot {% if alert_active %}alert{% else %}idle{% endif %}"></div>
    <div class="status-label {% if alert_active %}alert{% else %}idle{% endif %}">
      {% if alert_active %}ALERT{% else %}ALL CLEAR{% endif %}
    </div>
    ...
    {% if members %}
      ...
      <div class="member-row">
        ...
        <div class="member-actions">
          {% if m.status == "pending" %}
          <form method="POST" action="/approve/{{ m.telegram_id }}"><button class="btn btn-approve">Approve</button></form>
          <form method="POST" action="/reject/{{ m.telegram_id }}"><button class="btn btn-reject">Reject</button></form>
          {% endif %}
          <form method="POST" action="/remove/{{ m.telegram_id }}" onsubmit="return confirm('Remove?')">
...
def get_poller_status():
    try:
        ...
        if ago < 120:
            ...
            return "warn", "last seen " + str(int(ago / 60)) + "m ago"
        else:
            return "dead", "no signal for " + str(int(ago / 60)) + "m"
    except Exception:
        return "dead", "never connected — poller may be down"
