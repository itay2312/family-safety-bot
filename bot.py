...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # New user — save as pending, ask for their name
...
async def handle_name_input(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Captures the name sent after /start"""
    uid = update.effective_user.id
    member = db.get_member(uid)
...
async def handle_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("approve:") or data.startswith("reject:"):
        ...
        if action == "approve":
            ...
            await ctx.bot.send_message(
                chat_id=uid,
                text="🎉 You've been approved! You'll now receive safety check-ins after rocket alerts."
            )
        else:
    ...
    if ":" in data:
        ...
        name = db.get_member(uid)["name"] if db.get_member(uid) else "Unknown"
...
async def poll_alerts(app: Application):
    global alert_state, last_alert_time, current_event_id
                ...
                async with session.get(
                    ...
                    if active:
                        ...
                        if elapsed >= COOLDOWN_SECS:
                            ...
                            await send_checkins(app.bot, event_id)
...
# SEND CHECK-INS
# ─────────────────────────────────────────
async def send_checkins(bot: Bot, event_id: str):
    ...
    for m in members:
        try:
            await bot.send_message(
                ...
                text=f"🚨 *Rocket alert just ended.*\n\nAre you safe, {m['name']}?",
                parse_mode="Markdown",
                reply_markup=keyboard
            )
        except Exception as e:
...
async def cmd_status(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not members:
        ...
        return
    lines = [f"🤖 *Bot Status* — Alert: {alert_state}\n"]
    for m in members:
...
async def cmd_test(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    current_event_id = event_id
    await send_checkins(ctx.bot, event_id)
